#ifndef MARIO_SRC_RT_DEBUG_H
#define MARIO_SRC_RT_DEBUG_H
...
extern const char* mario_log_prefix_;

#define __SHORT_FILE__ (strrchr(__FILE__, '/') ? strrchr(__FILE__, '/') + 1 : __FILE__)

#ifdef _TRACE_ENABLE
#define  _trace(fmt, ...) do { printf( BLUE "[T] %s [%s:%d:%s] " fmt RESET "\n", mario_log_prefix_, __SHORT_FILE__, __LINE__, __func__, __VA_ARGS__); fflush(stdout); } while (0)
...
#ifdef _CHECK_ENABLE
#define  _check() do { printf( PURPLE "[C] %s [%s:%d:%s]" RESET "\n", mario_log_prefix_, __SHORT_FILE__, __LINE__, __func__); fflush(stdout); } while (0)
...
#define ___error(fmt, ...) do { printf(_RED "[E] %s [%s:%d:%s] " fmt RESET "\n", mario_log_prefix_, __SHORT_FILE__, __LINE__, __func__, __VA_ARGS__); fflush(stdout); } while (0) // MacOS
#else
...
#define  _clerror(err) do { if (err != CL_SUCCESS) { printf( RED "[E] %s [%s:%d:%s] err[%d]" RESET "\n", mario_log_prefix_, __SHORT_FILE__, __LINE__, __func__, err); fflush(stdout); } } while (0)
